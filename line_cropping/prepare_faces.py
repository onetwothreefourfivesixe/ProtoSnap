"""Phase 2: face preparation.
  ...
  python -m line_cropping.prepare_faces ebl   -> cut each annotated side out of the eBL composite photos
  python -m line_cropping.prepare_faces oa    -> accepted-face list for the Old Assyrian set, edges rotated
...
def tablet_components(im, scale=6, min_area_frac=0.002):
    ...
    thr = min(60, int(_)) if _ else 40                       # Otsu, but never higher than 60: shadows on the tablet must stay inside
...
def ebl():
    ...
    for (tab, script, side, obj), bs in sorted(by_face.items()):
        ...
        rows.append(dict(face=name, tablet=tab, script=script, side=side, object=obj, file=os.path.join(out, name + '.jpg'), crop_method=method,
